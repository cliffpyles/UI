...
describe("Delta", () => {
  it("absolute positive", () => {
    render(<Delta current={110} previous={100} data-testid="d" locale="en-US" />);
    expect(screen.getByTestId("d")).toHaveTextContent("+10");
    expect(screen.getByTestId("d")).toHaveClass("ui-delta--up");
  });

  it("absolute negative", () => {
    render(<Delta current={90} previous={100} data-testid="d" locale="en-US" />);
    expect(screen.getByTestId("d")).toHaveTextContent("\u221210");
    expect(screen.getByTestId("d")).toHaveClass("ui-delta--down");
  });

  it("percent change", () => {
    render(<Delta current={110} previous={100} format="percent" data-testid="d" locale="en-US" />);
  ...
  it("em-dash for percent when previous is 0", () => {
    render(<Delta current={10} previous={0} format="percent" />);
    expect(screen.getByText("\u2014")).toBeInTheDocument();
  });

  it("invert flips coloring", () => {
    render(<Delta current={110} previous={100} invert data-testid="d" />);
    expect(screen.getByTestId("d")).toHaveClass("ui-delta--down");
  });

  it("forwards ref, merges className", () => {
    const ref = createRef<HTMLSpanElement>();
    render(<Delta ref={ref} current={1} previous={0} className="x" data-testid="d" />);
